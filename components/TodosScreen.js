...
import { getSingleTodo } from "./Redux/Actions/todoAction";

const Todos = () => {
  const todos = useSelector((state) => state.todos);
  let singleTodo = useSelector((state) => state.singleTodo);
  ...
  const handleOnChangeText = (text) => {
    ...
    // const newTodo = {
    //   ...todo,
    //   todo: trimmedText.length > 0 ? trimmedText : " ",
    // };
    // setTodo(newTodo);
  };
  ...
  useEffect(() => {
    ...
    // console.log("This is in UseEffect : ", todo);
  }, [singleTodo]);

  const handleOnPress = () => {
    if (todo.todo !== "") {
      // console.log("This is ID : ", todo.id);
  ...
  const updateTodo = (index) => {
    dispatch(getSingleTodo(index));
  };

  // console.log("While excuting todo : ", todo);
